...
def load_json_config(file_path: str) -> Dict[str, Dict[str, Dict[str, Any]]]:
...
def save_json_config(config: Dict[str, Dict[str, Dict[str, Any]]], file_path: str) -> None:
    """Save the configuration to the JSON file."""
    with open(file_path, 'w') as file:
        json.dump(config, file, indent=2)

def get_groups(config: Dict[str, Dict[str, Dict[str, Any]]]) -> List[str]:
    """Return a list of all groups in the configuration."""
    return list(config.keys())

def get_servers_in_group(config: Dict[str, Dict[str, Dict[str, Any]]], group: str) -> List[str]:
    """Return a list of servers in the specified group."""
    return list(config[group].keys())

def get_server_config(config: Dict[str, Dict[str, Dict[str, Any]]], group: str, server: str) -> Dict[str, Any]:
    """Return the configuration for a specific server in a group."""
    return config[group][server]

def add_group(config: Dict[str, Dict[str, Dict[str, Any]]], group: str) -> None:
    """Add a new group to the configuration."""
    if group in config:
        raise ValueError(f"Group '{group}' already exists.")
    config[group] = {}

def add_server(config: Dict[str, Dict[str, Dict[str, Any]]], group: str, server: str, server_config: Dict[str, Any]) -> None:
...
def remove_group(config: Dict[str, Dict[str, Dict[str, Any]]], group: str) -> None:
    """Remove a group from the configuration."""
    if group not in config:
        raise ValueError(f"Group '{group}' does not exist.")
    del config[group]

def remove_server(config: Dict[str, Dict[str, Dict[str, Any]]], group: str, server: str) -> None:
...
def edit_group(config: Dict[str, Dict[str, Dict[str, Any]]], old_group: str, new_group: str) -> None:
...
def edit_server(config: Dict[str, Dict[str, Dict[str, Any]]], group: str, old_server: str, new_server: str, new_config: Dict[str, Any]) -> None:
...
def create_sample_config() -> Dict[str, Dict[str, Dict[str, Any]]]:
